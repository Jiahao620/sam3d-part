...
def load_model_part(config_file: str, device: str = 'cpu', compile: bool = False):
    config = OmegaConf.load(config_file)
    config.rendering_engine = "pytorch3d"  # overwrite to disable nvdiffrast
    config.compile_model = compile
    config.workspace_dir = os.path.dirname(config_file)
    config.device = device
    # config._target_ = "sam3d_objects.pipeline.training_pipeline_pointmap.TrainingPartSSPipeline"
    # config._target_ = "sam3d_objects.pipeline.training_pipeline_pointmap.TrainingPartSSPipeline_ultrashape"
    # config._target_ = "sam3d_objects.pipeline.training_pipeline_pointmap.TrainingPartSSPipeline_compress"
    # config._target_ = "sam3d_objects.pipeline.training_pipeline_pointmap.TrainingPartSSPipeline_compress_ultrashape_pos"
    config._target_ = "sam3d_objects.pipeline.training_pipeline_pointmap.TrainingPartSSPipeline_compress_ultrashape_pos_cache"
    # config.ss_preprocessor.normalize_pointmap = False
    # config.ss_preprocessor.img_mask_pointmap_joint_transform = []
    # config.ss_preprocessor.img_transform = []
    # config.ss_preprocessor.mask_transform = []
    # config.ss_preprocessor.pointmap_transform = []
    check_hydra_safety(config, WHITELIST_FILTERS, BLACKLIST_FILTERS)
    # pipeline: TrainingPartSSPipeline = instantiate(config)
    # pipeline: TrainingPartSSPipeline_ultrashape = instantiate(config)
    # pipeline: TrainingPartSSPipeline_compress_ultrashape_pos = instantiate(config)
    pipeline: TrainingPartSSPipeline_compress_ultrashape_pos_cache = instantiate(config)
    return pipeline

def load_model_part_hunyuan3d(config_file: str, device: str = 'cpu', compile: bool = False):
    config = OmegaConf.load(config_file)
    config.rendering_engine = "pytorch3d"  # overwrite to disable nvdiffrast
    config.compile_model = compile
    config.workspace_dir = os.path.dirname(config_file)
    config.device = device
    config._target_ = "sam3d_objects.pipeline.training_pipeline_pointmap.TrainingPartSSPipeline_compress_hunyuan3d_pos_cache"
    check_hydra_safety(config, WHITELIST_FILTERS, BLACKLIST_FILTERS)
    pipeline = instantiate(config)
    return pipeline

def load_model_part_hunyuan3d_new_vae(config_file: str, device: str = 'cpu', compile: bool = False):
    config = OmegaConf.load(config_file)
    config.rendering_engine = "pytorch3d"
    config.compile_model = compile
    config.workspace_dir = os.path.dirname(config_file)
    config.device = device
    config._target_ = "sam3d_objects.pipeline.training_pipeline_pointmap.TrainingPartSSPipeline_compress_hunyuan3d_new_vae"
    check_hydra_safety(config, WHITELIST_FILTERS, BLACKLIST_FILTERS)
    pipeline = instantiate(config)
    return pipeline
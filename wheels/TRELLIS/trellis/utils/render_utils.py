...
def render_frames(sample, extrinsics, intrinsics, options={}, colors_overwrite=None, verbose=True, **kwargs):
    ...
    for j, (extr, intr) in tqdm(enumerate(zip(extrinsics, intrinsics)), desc='Rendering', disable=not verbose):
        if isinstance(sample, MeshExtractResult):
            return_types = ["mask", "normal", "pointmap"]
            # Enable color rendering if vertex_attrs or texture is available
            if sample.vertex_attrs is not None or (sample.uv is not None and sample.texture_map is not None):
                return_types.append("color")
            res = renderer.render(sample, extr, intr, return_types=return_types)
            for key in ['normal', 'color', 'pointmap', 'mask']:
                if key in res:
                    if key not in rets:
                        rets[key] = []
                    val = res[key].detach().cpu().numpy()
                    if key == 'color':
                        rets[key].append(np.clip(val.transpose(1, 2, 0) * 255, 0, 255).astype(np.uint8))
                    elif key == 'normal':
                        rets[key].append(val)
                    elif key == 'pointmap':
                        rets[key].append(val)

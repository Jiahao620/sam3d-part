...
class MeshRenderer:
    ...
    def render(
        ...
        Args:
            ...
            return_types (list): list of return types, can be "mask", "depth", "normal_map", "normal", "color", "pointmap"
        ...
        Returns:
            edict based on return_types containing:
                ...
                pointmap (torch.Tensor): [3, H, W] rendered world-space point positions
        """
        ...
        if mesh.vertices.shape[0] == 0 or mesh.faces.shape[0] == 0:
            ...
            ret_dict = {k : default_img if k in ['normal', 'normal_map', 'color', 'pointmap'] else default_img[..., :1] for k in return_types}
            return ret_dict
        ...
        for type in return_types:
            ...
            if type == "mask" :
                ...
                if mesh.uv is not None and mesh.texture_map is not None:
                    # UV-mapped texture rendering
                    uv_attr = mesh.uv.contiguous()  # (V, 2)
                    uv_interp, _ = dr.interpolate(uv_attr, rast, faces_int)  # (1, H, W, 2)
                    # Sample texture using grid_sample: texture_map is (H_tex, W_tex, 3)
                    tex = mesh.texture_map.permute(2, 0, 1).unsqueeze(0)  # (1, 3, H_tex, W_tex)
                    # Convert UV [0,1] to grid_sample coords [-1,1]
                    grid = uv_interp * 2.0 - 1.0  # (1, H, W, 2)
                    # Flip V coordinate (OpenGL UV convention: V=0 at bottom)
                    grid[..., 1] = -grid[..., 1]
                    img = F.grid_sample(tex, grid, mode='bilinear', padding_mode='border', align_corners=False)  # (1, 3, H, W)
                    img = img.permute(0, 2, 3, 1)  # (1, H, W, 3)
                    img = dr.antialias(img.contiguous(), rast, vertices_clip, faces_int)
                else:
                    # Vertex color rendering
                    img = dr.interpolate(mesh.vertex_attrs[:, :3].contiguous(), rast, faces_int)[0]
                    img = dr.antialias(img, rast, vertices_clip, faces_int)
            elif type == "pointmap" :
                # World-space position (xyz) per pixel
                img = dr.interpolate(mesh.vertices.contiguous(), rast, faces_int)[0]
                img = dr.antialias(img, rast, vertices_clip, faces_int)
